...
// Global variables
	...
	var propositionsInit = function($containerAjax){	// ф-я ініціалізації js-функціоналу на підвантаженому блоці пропозицій
		var 
			...
			,$coverageSelect = $vehicleForm.find("#coverageArea")
			,$coverageArea = $vehicleForm.find("#coverageArea option:selected")
			,$contractSelect = $vehicleForm.find("#contractPeriod")
			,$contractPeriod = $vehicleForm.find("#contractPeriod option:selected")
			...
			$proposListContainer.queue("ajax", function(){
				...
				var changeHeader =function($toggleFilters){
					var 
						...
						,area = selectVal($('#coverageArea'))
						,period = selectVal($('#contractPeriod'))
						,headerText = sVehicle + ' / ' + area + ' / ' + period
						;
		...
		// 	$vehicleForm.trigger("submit");
		// });
		$coverageSelect.on("change", function(){
			$vehicleForm.trigger("submit");
		});
